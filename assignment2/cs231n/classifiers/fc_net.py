    ...
    def loss(self, X, y=None):
        ...
        if self.use_batchnorm:
            for i in range(1,self.num_layers):
                scores[i],aff_cache[i]=affine_forward(scores[i-1],self.params['W%d'%(i)],self.params['b%d'%(i)])
                scores[i],bn_cache[i]=batchnorm_forward(scores[i],self.params['gamma%d'%(i)],self.params['beta%d'%(i)],self.bn_params[i-1])
                scores[i],relu_cache[i]=relu_forward(scores[i])
                if self.use_dropout:
                    scores[i],dropout_cache[i]=dropout_forward(scores[i],self.dropout_param[i])
        else:
            for i in range(1,self.num_layers):
                scores[i],relu_cache[i]=affine_relu_forward(scores[i-1],self.params['W%d'%(i)],self.params['b%d'%(i)])
                if self.use_dropout:
                    scores[i],dropout_cache[i]=dropout_forward(scores[i],self.dropout_param)
        scores[self.num_layers],aff_cache[self.num_layers]=affine_forward(scores[self.num_layers-1],self.params['W%d'%(self.num_layers)],self.params['b%d'%(self.num_layers)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        ###########################################################################
        loss,dout=softmax_loss(scores[self.num_layers],y)
        dout,grads['W%d'%(self.num_layers)],grads['b%d'%(self.num_layers)]=affine_backward(dout,aff_cache[self.num_layers])
        ...
        if self.use_batchnorm: 
            for i in range(self.num_layers-1,0,-1):
                ...
                dout,grads['W%d'%(i)],grads['b%d'%(i)]=affine_backward(dout,aff_cache[i])
                grads['W%d'%(i)]+=self.reg*self.params['W%d'%i]
        else:
            for i in range(self.num_layers-1,0,-1):
                if self.use_dropout:
                    dout=dropout_backward(dout,dropout_cache[i])
                dout,grads['W%d'%(i)],grads['b%d'%(i)]=affine_relu_backward(dout,relu_cache[i])
                grads['W%d'%(i)]+=self.reg*self.params['W%d'%i]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
